# P2 Multimodal Fusion — 멀티모달 응급 원인 융합 (Project 2)
...
확인했다. 이 한계가 후속 cross-modal attention과 실데이터 검증 시스템의 동기가 되었다.

설계 근거·결과·한계의 전체 서술은 [`PROJECT_JOURNEY.md`](PROJECT_JOURNEY.md), 기록 인덱스는
[`decisions.md`](decisions.md). 교차 어텐션 접근도 본 레포에서 함께 다루며, 합성 데이터에선 late fusion
대비 우위가 없어 실 정렬 데이터 검증 후로 이월한다.
