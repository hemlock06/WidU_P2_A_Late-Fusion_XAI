...
def measure(p):
    ...
    GW=[];LB=[];MK=[]
    for b in test:
        b={k:v.to(DEV) for k,v in b.items()}; o=m(b)
...
def rep(path,tag):
    ...
    for ci,n in enumerate(CLS):
        ...
        selA=(LB==ci); selF=(LB==ci)&(MK.sum(1)>=2.999)
        W(f"  {n:8s} {MOD[p]:4s}  as-is={GW[selA].mean(0)[p]:.3f}(n={int(selA.sum())})  full={GW[selF].mean(0)[p]:.3f}(n={int(selF.sum())})")

def pick(model,**kw):
    for p in sorted(glob.glob(CKR+rf"\p2_{model}_*\best_model.pt")):
        a=torch.load(p,map_location="cpu",weights_only=False).get("args",{})
        if all(a.get(k)==v for k,v in kw.items()): return p
    return None
rep(pick("gated",dataset_version="vf",gate_mode="conf_routed",emb_bottleneck=16),"GMU bn16 vf")
rep(pick("cross_attn",dataset_version="vf",emb_bottleneck=32,epochs=80),"cross_attn bn32 vf")
