...
def build(a):
    if a["model"] == "cross_attn":
        m = CrossModalAttentionFusion(d_model=a["d_model"], n_heads=a["n_heads"],
              n_layers=a["n_layers"], dropout=a["dropout"],
              aux_loss_weight=a["aux_weight"], emb_bottleneck=a.get("emb_bottleneck", 16))
    else:
        m = GatedFusionModel(fusion_hidden=(256,128), dropout=a["dropout"],
              aux_loss_weight=a["aux_weight"], gate_input_norm=True,
              fusion_level=a.get("fusion_level","feature"), gate_mode=a.get("gate_mode","learned"),
              temperature=a.get("temperature",0.15), emb_bottleneck=a.get("emb_bottleneck",0))
    return m.to(DEV)
...
def measure(ckpt_path):
    ...
    _, _, test = make_loaders(DATA, batch_size=1024, modality_dropout_p=0.0,
    ...
    GW=np.concatenate(GW); LB=np.concatenate(LB); MK=np.concatenate(MK)
    return GW, LB, MK, a
...
def per_class_reliance(GW, LB, MK, full_only=False):
    ...
    for ci,name in enumerate(CLS):
        sel = (LB==ci) & full
        if sel.sum()==0: res[name]=(None,None,0); continue
        mg = GW[sel].mean(0)  # [3]
        p = PRIMARY[ci]
...
for p in ca:
    ...
    if c.get("args",{}).get("emb_bottleneck")==32 and c.get("args",{}).get("epochs")==80:
        ca_valid.append(p)
W(f"=== CROSS_ATTN (vf, bn=32, {len(ca_valid)} seeds) ===")
acc={name:[] for name in CLS}; accv={name:[] for name in CLS}
for p in ca_valid:
    GW,LB,MK,a=measure(p)
    r=per_class_reliance(GW,LB,MK,full_only=True)
    sid=os.path.basename(os.path.dirname(p))
    W(f"[{sid}] " + "  ".join(f"{n}:{(r[n][0] if r[n][0] is not None else -1):.3f}" for n in CLS))
    for n in CLS:
        if r[n][0] is not None: acc[n].append(r[n][0])
        if r[n][1] is not None: accv[n].append(r[n][1])
W("--- MEAN (full-modality test) primary-mod reliance ---")
for n in CLS:
    if acc[n]:
        W(f"  {n:8s} primary={np.mean(acc[n]):.3f} ± {np.std(acc[n]):.3f}   mean_vec(ECG,IMU,SpO2)={[round(x,3) for x in np.mean(accv[n],0)]}")
...
for tag,bn in [("GMU bn=16",16),("GMU bn=32",32)]:
    ...
    pick=None
    for p in g:
        c=torch.load(p,map_location="cpu",weights_only=False); a=c.get("args",{})
        if a.get("dataset_version")=="vf" and a.get("gate_mode")=="conf_routed" and a.get("emb_bottleneck")==bn:
            pick=p; break
    if not pick: W(f"=== {tag}: none ==="); continue
    GW,LB,MK,a=measure(pick)
    r=per_class_reliance(GW,LB,MK,full_only=True)
    W(f"=== {tag} ({os.path.basename(os.path.dirname(pick))}) full-mod primary reliance ===")
    for n in CLS:
        if r[n][0] is not None:
            W(f"  {n:8s} primary={r[n][0]:.3f}   vec(ECG,IMU,SpO2)={[round(x,3) for x in r[n][1]]}  n={r[n][2]}")
out.close()
