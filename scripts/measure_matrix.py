...
def per_class_mat(p):
    ...
    for b in test:
        ...
        A.append(o["attention_weights"].cpu().numpy()); L.append(b["label"].cpu().numpy())
    A=np.concatenate(A);L=np.concatenate(L)
    return {ci: A[L==ci].mean(0) for ci in range(5)}  # ci-> [3,3]

paths=[p for p in sorted(glob.glob(CKR+r"\p2_cross_attn_*\best_model.pt"))
       if torch.load(p,map_location="cpu",weights_only=False).get("args",{}).get("emb_bottleneck")==32
       and torch.load(p,map_location="cpu",weights_only=False).get("args",{}).get("epochs")==80]
