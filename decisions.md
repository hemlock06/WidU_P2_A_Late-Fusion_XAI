...
> 본 레포의 cross-modal attention 비교(합성 데이터선 우위 없음)와 실 정렬 데이터 검증·통합으로
...
## 3-프로젝트 로드맵 (P2: late fusion 채택, cross-modal attention 비교)

| 프로젝트 | 역할 | 입력 | 출력 | 상태 |
|---|---|---|---|---|
| P1 (ECG) | 심전도 분석 | 12-lead ECG | 심장분류 + 응급점수 + 임베딩 + 신뢰도 | ✅ 완료 |
| **late fusion (본 레포)** | 말단 결합 융합 | P1 출력 + IMU + SpO2 | 응급 원인 5분류 | ✅ 채택·결론 |
| cross-modal attention (비교군) | 교차 어텐션 융합 | P1 임베딩 + IMU + SpO2 | 응급 원인 분류 | 본 레포 · 합성선 우위 없음 |
...
| 함의 | 교차모달 학습은 실 정렬 데이터 필요 / 신뢰 가능 시스템은 실데이터 검증 검출기 + 투명 결합 |
